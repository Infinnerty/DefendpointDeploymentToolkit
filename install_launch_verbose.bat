...
goto :eof
:run
START /B /WAIT "Client Deployment" powershell -ex Bypass -WindowStyle Normal -Command "& { & '.\Deploy-Application.ps1' -DeploymentType Install -IC3Mode $false -ShowWelcomePromptWithDefer $false -ForceUninstallPreviousVersions $false -ReplaceCurrentClient $false -CustomActions $false; Exit $LastExitCode }
Pause
EXIT %errorlevel%
