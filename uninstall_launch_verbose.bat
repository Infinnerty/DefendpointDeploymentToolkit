...
goto :eof
:run
START /B /WAIT "Client Deployment" powershell -ex Bypass -WindowStyle Normal -Command "& { & '.\Deploy-Application.ps1' -DeploymentType Uninstall -IC3Mode $false -ShowWelcomePromptWithDefer $false -ForceUninstallPreviousVersions $true -ReplaceCurrentClient $true; Exit $LastExitCode }
Pause
EXIT %errorlevel%
